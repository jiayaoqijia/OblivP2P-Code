...
class BtClient:
    ...
    def readdata_1( self, peerid, dataid ):
        #fetch time
        self.btpeer.fetchstarttime = time.time()
        host,port = peerid.split(':')
        ...
        #print("self.btpeer.fetchcircuitnum", self.btpeer.fetchcircuitnum)
...
def main():
    ...
    for i in range(uploadN):
        app.uploaddata_1(peerid, "data"+str(i))

    accessN = 9
    readsequence = []
    for i in range(accessN):
        n = random.randint(0, uploadN-1)
        readsequence.append("data"+str(n))

    count = 0
    for i in readsequence:
        count += 1
        print("fetch data", i, count)
        app.readdata_1(peerid, i)
