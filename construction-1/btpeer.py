...
class BTPeer:
    ...
    #--------------------------------------------------------------------------
        ...
        #for construction-1
        self.dummypath = "."
        self.N = 4 #leaves number: 2**N
        self.Z = 4
        #S = 6
        self.C = 3
        self.A = 3
        self.blocksize = 1024 * 1
        self.prerandompointsnum = 1
        self.btitpiroram = BtITPIRORAM(self.Z, ".", ".", self.N, self.A, self.C, self.blocksize, self.prerandompointsnum )

        self.bucketdata = []
        self.stashpeerdata = []

        self.uploadcircuitpeers = {} #transactionid --> circuit peers[]
        self.transactions = {} #transactionid --> 
        self.uploadsecretpoints = {} #transactionid --> secret points
        self.uploaddatashares = {} #transactionid --> data shares
        self.stashpeerid = "" #stashpeerid
        self.uploadstashshares = {} #transactionid --> {circuitid --> share}
        self.uploadstashsharesnum = 0
        self.uploadstashcircuitpeers = {} #transactionid --> circuit peers[]
        self.trackerid = "" #trackerid

        self.fetchcircuitpeers = {} #transactionid --> circuit peers[]
        self.fetchinitiator = {} #transactionid --> initiator
        self.fetchstashpeerid = {} #transactionid --> stashpeer
        self.fetchpathpeernum = {} #transactionid --> num
        self.fetchcircuitinfo = {} #transactionid --> [oldpoint, newpoint, []]
        self.fetchcircuitdata = {} #transactionid --> [datafromR, datafrom0, ..., datafromstash]
        self.fetchcircuitnum = {} #transactionid --> num
        self.fetchinitcircuitdata = {}#transactionid --> {peerid --> data}
        self.fetchstashcircuitnum = {} #transactionid --> num
        self.fetchstashcircuitdata = {}#transactionid --> {peerid --> data}

        self.evictioncircuitpeers = {} #transactionid --> circuit peers[]
        self.evictionpathpeernum = {} #transactionid --> num
        self.evictioncircuitinfo = {} #transactionid --> [oldpoint, newpoint, []]
        self.evictioncircuitdata = {} #transactionid --> [datafromR, datafrom0, ..., datafromstash]
        self.evictionpathpeercircuitdata = {} #transaction --> {circuitpeerid --> [points0, points1, ...]}
        self.evictioncircuitpeernum = {} #transactionid --> num
        self.evictionstashpeercircuitdata = {} #transaction --> {circuitpeerid --> [points0, points1, ...]}
...
class BTPeerConnection:
    ...
    #--------------------------------------------------------------------------
        ...
        try:
            ...
            #msg = "4"
            #print("msg before decoding: %s" % msg)
            try:
                #msg = msg.decode("utf-8", "ignore")
                msg = msg.decode("ISO-8859-1")
            except Exception as e:
            #'utf-8' codec can't decode byte 0x80 in position 7: invalid start byte
            #Fixed bug: if size == 1408, decoding doesn't work.
                print("Couldn't do it: %s" % e)
            #print("msg before sending: %s" % msg)
            self.sd.write( msg )
            self.sd.flush()
        except KeyboardInterrupt:
            raise
